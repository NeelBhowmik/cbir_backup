 ...
 */
//Access IUI All:
//g++ RetrieveImages_POEME_2.cpp -L /usr/lib -lopencv_core -lopencv_flann -o RetrieveImages_POEME_2 -std=c++11
...
void getMaxMinDist(distances &dists, int k, string &root, string &dataset, string descriptor, string cb_size) {
	...
	string str = ss.str();
	float testDist;	
	string path = root + "/3.5_ComputeDistances/data/" + dataset + "/" + descriptor + "/" + cb_size + "/Distances" + str + "nn.dat";
...
void readIUI(vector<vector<string> > &IUI, string &root, string &dataset, string &descriptor, string &cb_size) {
	string path = root + "/3_InvertedUniqueIndices/data/" + dataset + "/" + descriptor + "/InvertedUniqueIndices" + cb_size + ".dat";
	string data_line;
	ifstream fp(path.c_str());
	int i = 0;
...
cv::flann::Index * loadKdtree(string &root, string &dataset, string &descriptor, string &cb_size, Mat &CodeBook) {
	string CodeBookPath = root + "/2_CodeBooks/data/" + dataset + "/" + descriptor + "/CodeBook" + cb_size + ".dat";
	string KdtreePath = root + "/2_CodeBooks/data/" + dataset + "/" + descriptor + "/Kdtree" + cb_size + ".dat";
...
void computeMSRank(string root, string dataset, string detector, string descriptor, string cb_size, string knn, int t, string list_classes, int verb) {
	...
	for (unsigned int i = 0; i < classes.size(); i++) {
		...
		/**************************************************************************/
		dir_in = root + "/1_Descriptors/data/" + dataset + "/cea+2/" + classes[i];
		...
		for (unsigned int j = 0; j < images.size(); j++) {
			...
			for (unsigned int k = 0; k < descriptors.size(); k++) {
				string path = root + "/1_Descriptors/data/" + dataset + "/" + descriptors[k] + "/" + classes[i] + "/" + img;
